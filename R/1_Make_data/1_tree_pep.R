...
pep_mes <- st_read("data/raw_data/PEP_GPKG/PEP.gpkg", layer = "placette_mes")

# tree DHP + status
tree_mes <- st_read("data/raw_data/PEP_GPKG/PEP.gpkg", layer = "dendro_arbres")
...
pep_mes <- pep_mes %>%
  ...
  select(id_pe, id_pe_mes, no_mes, year_measured)
...
tree_data0 <- pep_mes %>%
  ...
  left_join(tree_mes, by = c("id_pe", "id_pe_mes", "no_mes"), multiple = "all")
...
last_sp_code <- tree_data %>%
  ...
  group_by(id_arbre) %>%
  slice_max(order_by = year_measured) %>%
  ungroup() %>%
...
pe_deleted <- tree_data0 %>%
  ...
  distinct(id_pe, id_pe_mes, year_measured, no_mes, state)
...
lonlat <- st_read("data/raw_data/PEP_GPKG/PEP.gpkg", layer = "placette") %>%
  select(id_pe, longitude, latitude)
pep_xy <- pep_xy %>% merge(data.frame(lonlat))

version_inv <- st_read("data/raw_data/PEP_GPKG/PEP.gpkg", layer = "placette_mes") %>%
  filter(id_pe %in% unique(tree_data$id_pe)) %>%
  select(id_pe_mes, version)

tree_data <- tree_data %>% merge(version_inv, all.x = TRUE)

saveRDS(tree_data, "data/data_mh/tree_data_fev2023.RDS")

saveRDS(growth, "data/data_mh/growth_data_fev2023.RDS")

saveRDS(sp_mat, "data/data_mh/sp_mat_abun_fev2023.RDS")
saveRDS(sp_BA, "data/data_mh/sp_mat_ba_fev2023.RDS")

# saveRDS(version_inv, "data/version_inv.RDS")

# ajouter l'altitude venant de pep_pe$altitude
pep_pe <- pep_pe %>% select(id_pe, altitude) %>%
  group_by(id_pe) %>% summarise(altitude = mean(altitude, na.rm = T))
