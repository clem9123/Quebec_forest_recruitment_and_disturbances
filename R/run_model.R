...
run_jags_model <- function(sp, model_file, filename = paste0(sp, ".rds"), 
    ...
    # 2. Load data
    jags_data <- make_jags_data(sp, devel)
    # 3. Run model
    param <- c(
        # ba (basal area)
        "ba_intercept",
    ...
    Tex <- Sys.time() - begin
    out$runtime <- Tex
    cli::cli_alert_info("computation ended: {Tex}")
...
run_analyses <- function(spec, model_file, output_dir = "output", devel = FALSE, ...) {
    ...
    parallel::parLapply(
        cl,
        list_species()[1:3],
        \(x) run_jags_model(x, model_file, output_dir, devel = TRUE, ...)
    )
...
make_jags_data <- function(sp, devel = FALSE) {
