...
myPalette3 <- colorRampPalette(c("white","#001b92", "#1ee4e7", "#14af1c", "#fffb00", "#e5a836", "#e5a836","#e26e10", "#e26e10","#c00606", "#c00606", "#670101", "#670101"))
```

```{r combination prediction plot}
combination_plot <-
    ggplot(wd) +
        aes(y = latitude, x = year_measured, fill = log(pred)) +
        geom_raster(interpolate = TRUE) +
        scale_fill_gradientn(colours = myPalette1(200), labels = function(x) format(x, scientific = TRUE)) +
        facet_wrap(~sp_code, labeller = as_labeller(c("ACERUB" = "A. rubrum", "ACESAC" = "A. saccharum",
                        "BETALL" = "B. alleghaniensis"))) +
    ...
    # limite entre les facettes
        ...
        labs(y = "Latitude", x = "Year", fill = "log(Sapling number)")
```
...
combination_plot2 <-
    ggplot(wd %>% filter(sp_code %in% c("ABIBAL", "PICMAR"))) +
        aes(y = latitude, x = year_measured, fill = pred) +
        geom_raster(interpolate = TRUE) +
        scale_fill_gradientn(colours = myPalette2(500), labels = function(x) format(x, scientific = TRUE)) +
...
combination_plot3 <-
    ggplot(wd %>% filter(sp_code %in% c("BETPAP"))) +
        aes(y = latitude, x = year_measured, fill = pred) +
        geom_raster(interpolate = TRUE) +
        scale_fill_gradientn(colours = myPalette2(500), labels = function(x) format(x, scientific = TRUE)) +
        facet_wrap(~sp_code, labeller = as_labeller(c("ABIBAL" = "A. balsamea", "PICMAR" = "P. mariana",
                        "PICGLA" = "P. glauca", "BETPAP" = "B. papyrifera", "POPTRE" = "P. tremuloides")), scale = "free") +
        scale_x_continuous(expand=c(0,0)) +
        scale_y_continuous(expand=c(0,0)) +
        # Ecrire les facettes en haut sans background
        theme(strip.background = element_blank(),
...
combination_plot4 <-
    ggplot(wd %>% filter(sp_code %in% c("POPTRE", "PICGLA"))) +
        aes(y = latitude, x = year_measured, fill = pred) +
        geom_raster(interpolate = TRUE) +
        scale_fill_gradientn(colours = myPalette2(500), labels = function(x) format(x, scientific = TRUE)) +
        facet_wrap(~sp_code, labeller = as_labeller(c("ABIBAL" = "A. balsamea", "PICMAR" = "P. mariana",
...
C#
"
combination_plot2 + combination_plot4 + combination_plot3 + plot_layout(design = layout)
```