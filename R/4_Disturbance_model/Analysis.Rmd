...
```{r}
source("R/4_Disturbance_model/function_analyses.R")
```
...
# Création d'un exemple de tableau de données
data <- data.frame(
  mean = c(1, 2, 3, 4, 5),
  min = c(0, 1, 2, 3, 4),
  max = c(2, 3, 4, 5, 6),
...
ggplot(data, aes(x = sp_code, y = mean)) +
  geom_point() +  # Utilisez geom_point() ou un autre type de géométrie souhaité
  facet_grid(var ~ ., scales = "free_x") +  # Organise les facettes par variable
  coord_flip() +  # Inverse les axes x et y
  theme(panel.background = element_rect(fill = "white"),  # Définit la couleur de fond du panneau
        panel.spacing = unit(0, "pt"),  # Ajuste l'espace entre les panneaux à zéro
        panel.border = element_blank(),  # Supprime les bordures par défaut des panneaux
        strip.background = element_blank(),  # Supprime le fond des étiquettes des facettes
        strip.placement = "outside",  # Place les étiquettes des facettes à l'extérieur du graphique
        strip.text.y = element_text(angle = 0, hjust = 0),  # Oriente le texte des étiquettes à 0 degré
        plot.margin = margin(5, 5, 5, 5, "pt")) +  # Ajuste les marges du graphique  # Dessine les bordures des facettes
...
# Plot the results
disturbance_plot(disturbance_pa, 10)
...
for (sp in Mysp){
    model <- get(paste0("Model_", sp))
    for (c in Dist_coef){
        df = data.frame(posterior = model$BUGSoutput$sims.list[paste0(c)][[1]][,2], sp_code = sp, coef = c,
...
ggplot(dist_posteriors) +# %>% filter(perturb %in% c("Clearcut", "Partial cut"))) +
    geom_violin(aes(y = posterior, x = sp_type, color = sp_code), alpha = 0.55) +
    facet_grid(part ~ perturb) +
    scale_fill_manual(name = "Species type", values = c("#2e35f9", "#0cd53e", "#df2222")) +
    theme_bw() +
    # legend position : bottom
