...
# DIC pour chaque model
#DIC2 <- sapply(Mysp, function(x) {
#    data.frame(get(paste0("Model_", x))$BUGSoutput$DIC)
#})
```
...
# Plot the results
disturbance_plot(disturbance_pa, 4)
...
## Correlation soil ph epmatorg

```{r}
library(corrplot)
full_data %>% mutate(soil = as.numeric(soil)) %>% select(soil, ph_humus, epmatorg) %>% cor() %>% corrplot(method = "number")
```